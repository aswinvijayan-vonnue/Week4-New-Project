...
movieContainer?.addEventListener("click", (e) => {
  ...
  const changeUserNameButton = movieContainer.querySelector(
    ".changeUserName-button",
  );
  const removeMovieButton = movieContainer.querySelector(
    ".remove-movie-button",
  );
...
document.addEventListener("submit", (e) => {
  ...
  if (target.closest("#movieForm")) {
    ...
    const form = target.closest(".changeUserNameForm");
    if (!form) return;
    const formData = new FormData(form);
    const data = Object.fromEntries(formData.entries());
    const username = data.username;
    if (username && username.trim() !== "") {
      const parentDiv = target.closest(".user-info-container");
